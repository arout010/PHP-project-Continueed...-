...
{ 
							...
							$q="select $search from addstatus where source='$var';";
		...
		<div class = "rightside">
			<section class="section">
				<form action="user_search_result.php" method="post">
					<label>Select End Stop</label>
					<select type="text" name="dest">
						<?php f1('destination');?>
					</select></br>
					<div class="clearfix">
						<button type="submit" class="signupbtn" name="Submit">Enter</button>
					</div></br></br>
					<label>State:<?php echo $_SESSION['state'];?></label></br>
					</br><label>Dist:<?php echo $_SESSION['dist'];?></label>
					</br><label>City:<?php echo $_SESSION['city'];?></label>
					</br><label>Source Stop:<?php echo $_SESSION['city'];?></label>
